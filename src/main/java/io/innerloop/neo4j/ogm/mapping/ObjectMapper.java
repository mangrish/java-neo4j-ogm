...
import io.innerloop.neo4j.ogm.mapping.converters.CurrencyConverter;
import io.innerloop.neo4j.ogm.mapping.converters.LocalDateTimeConverter;
import io.innerloop.neo4j.ogm.mapping.converters.ZoneIdConverter;
import io.innerloop.neo4j.ogm.mapping.converters.YearMonthConverter;
import io.innerloop.neo4j.ogm.mapping.converters.UUIDConverter;
...
public class ObjectMapper
{
    ...
    private static Map<MetaDataLabelKey, NodeMetaData> nodesByOrderedLabels = new HashMap<>();

//    private static Map<String, RelationshipMetaData> relationships = new HashMap<>();
    ...
    public ObjectMapper(String... packages)
    {
        ...
        for (Class<?> n : ns)
        {
            ...
            MetaDataLabelKey key = new MetaDataLabelKey(labelArray);
...
//    }
    ...
    public <T> List<T> loadAll(Class<T> type, Graph graph)
    {
        ...
        for (Node node : graph.getNodes())
        {
            ...
            Arrays.sort(labels);
            MetaDataLabelKey key = new MetaDataLabelKey(labels);
